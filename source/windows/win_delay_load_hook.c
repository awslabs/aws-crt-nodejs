...
FARPROC WINAPI delayHook(unsigned dliNotify, PDelayLoadInfo pdli) {
    switch (dliNotify) {
        case dliNotePreLoadLibrary:
            ...
            // As in Electron 4.x and higher, the symbols needed by native modules are exported by `electron.exe` instead of `node.exe`. It is necessary to overwrite 
            // the node load process. More info about Electron windows delay load issue: 
            //     https://www.electronjs.org/docs/latest/tutorial/using-native-node-modules#a-note-about-win_delay_load_hook
            
            // If we call GetModuleHandle with NULL, GetModuleHandle returns a handle to the file used to create the calling process (.exe file). This means if we 
            // launch the library through `node.exe`, GetModuleHandle would still return `node.exe`. While in the case of electron, instead of loading `node`, GetModuleHandle
