...
napi_value aws_napi_io_client_bootstrap_new(napi_env env, napi_callback_info info) {
    ...
    bootstrap_options.event_loop_group = aws_napi_get_node_elg();
    bootstrap_options.host_resolver = &binding->resolver;

    binding->bootstrap = aws_client_bootstrap_new(allocator, &bootstrap_options);
