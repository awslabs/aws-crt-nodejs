...
async function makeConnection(will?: MqttWill, client_id: string = `mqtt-unit-test-${uuid()}`) : Promise<MqttClientConnection> {
...
test_env.conditional_test(test_env.AWS_IOT_ENV.mqtt311_is_valid_iot_cred())('MQTT Will', async () => {
    await retry.networkTimeoutRetryWrapper( async () => {
        let errorEventWrapper: Promise<void> = new Promise(async (resolve, reject) => {
            ...
            connectionWithWill.on('error', (e) => { reject(e); });
            ...
            connectionWaitingForWill.on('error', (e) => { reject(e); });
            ...
            connectionDuplicate.on('error', (e) => { reject(e); });
