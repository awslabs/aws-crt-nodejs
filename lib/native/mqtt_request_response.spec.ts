...
test_env.conditional_test(test_env.AWS_IOT_ENV.mqtt5_is_valid_mtls_rsa())('GetShadow Success Rejected Mqtt5', async () => {
    await do_get_named_shadow_success_rejected_test(ProtocolVersion.Mqtt5, true);
});

test_env.conditional_test(test_env.AWS_IOT_ENV.mqtt5_is_valid_mtls_rsa())('GetShadow Success Rejected Mqtt311', async () => {
    await do_get_named_shadow_success_rejected_test(ProtocolVersion.Mqtt311, true);
});

test_env.conditional_test(test_env.AWS_IOT_ENV.mqtt5_is_valid_mtls_rsa())('GetShadow Success Rejected No CorrelationToken Mqtt5', async () => {
    await do_get_named_shadow_success_rejected_test(ProtocolVersion.Mqtt5, false);
});

test_env.conditional_test(test_env.AWS_IOT_ENV.mqtt5_is_valid_mtls_rsa())('GetShadow Success Rejected No CorrelationToken Mqtt311', async () => {
